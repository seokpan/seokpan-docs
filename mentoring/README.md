...
5. **Baseline을 보존하면서 현재 상태까지 추적할 수 있게 합니다.**
   - 멘토링 문서에 기록된 기준 시점의 상태는 이후 구현 결과로 덮어쓰지 않습니다.
   - 이후 실제로 결정·구현·운영 기준이 달라진 항목은 문서의 후속 상태 기록, GitHub 근거 색인, [PROJECT_CHANGES.md](../PROJECT_CHANGES.md)와 관련 Repository의 Issue/PR을 통해 현재 기준까지 이어지게 합니다.
   - `당시 상태`와 `현재 상태`가 다르다는 이유만으로 과거 문장을 고쳐서 당시 기록을 왜곡하지 않습니다.
   - 후속 변화가 없는 항목에는 불필요한 현재 상태 안내를 추가하지 않습니다.
