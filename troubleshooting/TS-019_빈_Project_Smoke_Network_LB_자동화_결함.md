...
# TS-019 — 기존 프로젝트 실행환경을 재사용하지 않은 새 환경의 최소 동작 검증(Smoke Test)에서 Network/LB 자동화의 NIC Ansible 수집 정보(Fact) 구조 가정과 Ansible Check Mode(실제 변경 없이 예상 결과를 확인하는 모드) 검증 결함이 드러남
...
기존 프로젝트의 `.venv`와 Ansible Collection을 재사용하지 않은 새 환경에서 버전 구성표(Version Matrix)를 재생성한 뒤 Common / Network / LB 최소 동작 검증(Smoke Test)을 수행했다.
...
따라서 Network 실제 실행 환경(Runtime) 장애가 아니라 **Fact Filter가 `ansible_facts` 전체 값을 순회하면서 각 값이 항상 `ipv4.address`를 가진 Mapping이라고 가정한 코드 결함**이다. 실제 Fact 안에는 List 등 다른 자료형도 존재하므로 새 실행환경/Fact 구조에서 이 가정이 깨졌다.
...
이 사례는 기존 실행 환경에서 우연히 정상 동작했다는 사실만으로 자동화의 재현성이 증명되지 않는다는 점을 보여준다. 특히 버전 고정(Version Lock) 이후 새 환경의 최소 동작 검증(Smoke Test)이 **숨겨진 자료형 가정과 Check Mode 검증 결함을 실제로 드러냈다.**
...
- `seokpan-infra` Issue #84의 Smoke Test에서 결함 발견까지는 완료
- 실제 Runtime은 정상
- Network/LB 코드 보완은 후속 작업으로 분리 예정
- 수정 후 기존 프로젝트 실행환경을 재사용하지 않은 새 환경에서 동일 Smoke Test를 다시 수행한 근거가 아직 없음

따라서 상태는 **진행 중**이다.

## 관련 근거

- 빈 환경 재현 및 Smoke Test Issue #84: https://github.com/seokpan/seokpan-infra/issues/84