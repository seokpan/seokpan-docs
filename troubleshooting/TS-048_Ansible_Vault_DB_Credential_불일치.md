...
현재 확인한 근거 기준으로는 PR #161의 `vault.yml` 변경 과정에서 DB Credential 3종이 의도하지 않게 다른 값으로 변경된 회귀로 판단했다.
...
```text
Vault 변경 범위        PASS
mariadb-01 일치 여부     PASS
mariadb-02 일치 여부     PASS
