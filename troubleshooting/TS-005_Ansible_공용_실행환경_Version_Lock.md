...
# TS-005 — Ansible 공용 실행환경이 버전 고정 없이 System 환경에 의존
...
| 항목 | 내용 |
|---|---|
| **시점** | 2026-08-28 시작, 2026-08-31 main 반영 |
| **상태** | **핵심 해결 · Clean Reproduction 일부 제한사항 존재** |
| **주 담당** | **이유빈 — 네트워크 및 공통 인프라·Ansible 통합** |
| **영향 역할** | 정태훈, 김상희, 최유준 전원 |
| **핵심 범주** | Ansible / Python / Version Lock / Reproducibility |
...
문제는 이 값들이 프로젝트에서 선택·검증한 Version Matrix가 아니라 **그냥 이미 설치되어 있던 환경**이었다는 점이다.
...
> **Repository 정의만으로 동일한 프로젝트 실행환경을 다시 만들 수 있게 하는 것**
...
System 환경은 Rollback용으로 남기고, Project `.venv`와 Collection 경로를 분리했다.

## 교차 회귀검증 중 발견된 별도 문제

이 작업은 단순 Version Lock에 그치지 않고 팀 영역별 회귀검증을 수행했다. 그 과정에서 Argo CD `ApplicationSet` Controller의 기존 `CrashLoopBackOff`가 발견됐지만, 이를 새 Version Matrix의 Regression으로 오판하지 않고 기존 Argo CD Bootstrap 문제로 분리했다. 해당 실제 장애는 **TS-007**에 별도 기록한다.
...
다만 그 이후 빈 Project 환경에서 수행한 Issue #84 Kubernetes Smoke에서도 `INJECT_FACTS_AS_VARS default to True is deprecated` 경고가 `kubernetes_validate` 계열 검증에서 다시 확인됐다. Smoke 자체는 `failed=0`, `unreachable=0`으로 통과했으므로 기능 장애는 아니지만, **PR #77이 Repository 전체의 같은 유형 참조를 모두 제거했다는 의미는 아니다.** 따라서 deprecated fact 정리는 추가 점검이 남은 호환성 보완 항목으로 유지한다.
...
빈 Project 환경에서 Bootstrap으로 Version Matrix를 재생성한 뒤 Smoke가 실제로 진행됐다. 최신 Issue #84 Evidence 기준:

- Kubernetes 주요 Playbook / Cluster Check Mode Smoke: **PASS**
...
- System Ansible Rollback 최종 검증: 아직 완료 Evidence 부족
- Kubernetes Smoke에서 `INJECT_FACTS_AS_VARS` 경고 재확인: 기능 PASS와 별개로 남은 deprecated fact 참조 추가 점검 필요

따라서 Version Matrix 재생성 자체는 확인됐지만:

> **Version Lock 구현 완료 = 전체 인프라 Clean Reproduction 100% 완료**
...
- Main Owner Issue #41: https://github.com/seokpan/seokpan-infra/issues/41
...
- Clean Reproduction Issue #84: https://github.com/seokpan/seokpan-infra/issues/84
- Issue #84 Smoke Evidence: https://github.com/seokpan/seokpan-infra/issues/84#issuecomment-5490758252
...
이 문제는 Project Ansible Version Matrix가 원인이 아니라 **새 실행환경 검증이 기존 수동 DB 권한 구성의 누락을 드러낸 부가 발견**이다. 별도의 장애 연쇄나 후속 자동화 PR이 확인되지 않아 독립 TS로 분리하지 않고 이 사례의 회귀검증 Evidence로만 보존한다.
