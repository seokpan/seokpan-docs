...
이 작업은 단순 버전 고정(Version Lock)에 그치지 않고 팀 영역별 회귀검증을 수행했다. 그 과정에서 Argo CD `ApplicationSet` Controller의 기존 `CrashLoopBackOff`가 발견됐지만, 이를 새 버전 구성표(Version Matrix) 때문에 새로 생긴 회귀 문제(Regression)로 오판하지 않고 기존 Argo CD 초기 구성 자동화(Bootstrap) 문제로 분리했다. 해당 실제 장애는 **TS-007**에 별도 기록한다.
...
다만 그 이후 기존 프로젝트 실행환경을 재사용하지 않고 새로 구성한 환경에서 수행한 `seokpan-infra` Issue #84의 Kubernetes 최소 동작 검증(Smoke Test)에서도 `INJECT_FACTS_AS_VARS default to True is deprecated` 경고가 `kubernetes_validate` 계열 검증에서 다시 확인됐다. Smoke Test 자체는 `failed=0`, `unreachable=0`으로 통과했으므로 기능 장애는 아니지만, **PR #77이 GitHub 저장소 전체의 같은 유형 참조를 모두 제거했다는 의미는 아니다.** 따라서 deprecated fact 정리는 추가 점검이 남은 호환성 보완 항목으로 유지한다.
...
기존 프로젝트 실행환경을 재사용하지 않은 새 환경에서 초기 구성 자동화(Bootstrap)로 버전 구성표(Version Matrix)를 재생성한 뒤 최소 동작 검증(Smoke Test)이 실제로 진행됐다. 최신 `seokpan-infra` Issue #84 근거 기준:

- Kubernetes 주요 Playbook / Cluster Ansible Check Mode(실제 변경 없이 예상 결과를 확인하는 모드) Smoke Test: **PASS**
- MariaDB / MaxScale / NFS Data·Storage Smoke Test: **PASS**
- Common `common_hosts`: PASS
- Network/LB: **TS-019의 NIC Fact / Check Mode 검증 결함으로 일부 FAIL**
- Argo CD 초기 구성 자동화(Bootstrap): **TS-018의 kubeconfig 경로·권한 불일치 때문에 검증 진행이 중단됨**
- System Ansible Rollback 최종 검증: 아직 완료 근거 부족
- Kubernetes Smoke Test에서 `INJECT_FACTS_AS_VARS` 경고 재확인: 기능 PASS와 별개로 남은 deprecated fact 참조 추가 점검 필요

따라서 버전 구성표(Version Matrix) 재생성 자체는 확인됐지만:

> **Version Lock 구현 완료 = 전체 인프라 빈 환경 재현 검증 100% 완료**

라고 표현하면 안 된다. 현재 남은 재현성 문제는 버전 고정 자체보다 Network/LB 자동화와 공용 kubeconfig 실행 경계 쪽으로 좁혀졌다.
...
이 문제는 프로젝트 Ansible 버전 구성표(Version Matrix)가 원인이 아니라 **새 실행환경 검증이 기존 수동 DB 권한 구성의 누락을 드러낸 부가 발견**이다. 별도의 장애 연쇄나 후속 자동화 PR이 확인되지 않아 독립 TS로 분리하지 않고 이 사례의 회귀검증 근거로만 보존한다.