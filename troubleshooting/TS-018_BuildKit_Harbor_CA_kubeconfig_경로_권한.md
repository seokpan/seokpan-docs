...
| **영향 범위** | 정태훈(Kubernetes 플랫폼·Argo CD 연동), 이유빈(Ansible Controller 접근 권한) |
...
즉 특정 팀원의 Home은 공용 초기 구성 자동화(Bootstrap) 자격증명 위치로 적절하지 않다.

## 현재 반영 방향과 실제 코드·검증 상태를 구분해야 함
...
다만 이 내용은 **설계 방향이 확정됐다는 사실과 실제 실행 환경(Runtime)에 모두 적용·검증됐다는 사실을 분리**해서 봐야 한다.

기존 프로젝트 실행환경을 재사용하지 않은 새 환경의 최소 동작 검증(Smoke Test)에서 확인한 실제 상태는 다음과 같다.
...
/etc/seokpan/kubeconfig/admin.conf
→ 최초 Smoke Test 시 Permission denied
```

따라서 당시 `argocd_bootstrap`은 첫 Kubernetes Task에서 **검증 진행이 중단**됐다.

후속 `seokpan-infra` Issue #90에서는 실제 Ansible Controller에서 다음 접근 권한까지 적용됐다.
...
참조가 남아 있다고 Issue #90에서 확인됐으므로, Role을 공용 `cluster_kubeconfig` 기준으로 맞춘 뒤 별도 Smoke Test 재실행이 여전히 필요하다.
...
- 공용 Role은 동일 변수 사용
- 자격증명(Credential) 내용은 Git에 저장하지 않음
...
```text
프로젝트 공용 초기 구성용 자격증명(Bootstrap Credential)
특정 팀원 개인 자격증명이 아님
공용 자동화가 사용
```
...
- `argocd_bootstrap`의 공용 `cluster_kubeconfig` 경로 정합화 및 Smoke Test 재실행 완료 근거는 아직 없음
