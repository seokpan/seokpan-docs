...
# TS-026 — MaxScale TLS 권한이 공용 Role 재실행 후 root-only 상태로 되돌아간 Runtime Drift
...
TS-025의 MaxScale `--check --diff` Credential 미노출 재검증 과정에서, 현재 `maxscale-01`의 TLS certificate/private key 권한이 MaxScale 서비스 계정이 읽을 수 없는 상태로 되돌아가 있는 것을 확인했다.
...
이번 사건은 그 정상화 이후 후속 `tls_deploy` 재실행/재발급 경로에서 Consumer-specific 권한이 다시 root-only 기본값으로 돌아갈 수 있었던 **Role 간 Desired State 충돌**이다.
...
이번 사건
→ 이후 공용 tls_deploy 재실행
→ 공용 Role의 root-only 기본 권한 재적용 가능
→ Consumer-specific 권한 유실
→ Runtime Drift
```
...
이면 정상화되지만, 이후 `tls_deploy`가 다시 certificate를 배포하거나 관련 권한을 적용하면 MaxScale이 필요로 하는 Consumer-specific 권한이 다시 유실될 수 있었다.

문제의 핵심은 단순한 파일 권한 오타가 아니라 **공용 Provider Role과 Consumer Role이 동일 자원에 서로 다른 Desired State를 선언한 구조**였다.
...
→ 후속 tls_deploy 재실행 시 root-only 권한으로 되돌아갈 수 있음
