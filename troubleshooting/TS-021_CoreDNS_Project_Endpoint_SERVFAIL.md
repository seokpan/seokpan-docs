...
| **영향 범위** | Kubernetes Pod의 프로젝트 FQDN(Fully Qualified Domain Name) 이름해석, Jenkins/BuildKit → Harbor 통신 경로 |
...
Jenkins/BuildKit의 Plugin Version, PriorityClass, BuildKit Image와 설정을 다시 확인한 뒤에도 동일한 `SERVFAIL`이 재현되어 Application이나 BuildKit 설정이 아니라 Kubernetes DNS 경로의 문제로 범위를 좁혔다.

## 원인 분석

Node와 Pod가 같은 도메인 이름을 서로 다른 방식으로 조회하고 있었다.
...
→ CoreDNS(10.96.0.10)
→ 상위 DNS로 전달
→ 상위 DNS에는 프로젝트 도메인 Record 없음
→ SERVFAIL
```

`harbor.seokpan.soldesk.store`는 당시 공용 DNS Server에 등록된 실제 Record가 아니라 Ansible `common_hosts`가 Node의 `/etc/hosts`에만 배포하는 프로젝트 내부 도메인이었다.

기존 05·06 설계에서는 Host의 `/etc/hosts`와 Pod가 사용하는 CoreDNS에 프로젝트 도메인을 각각 제공하도록 구분했지만, 실제 구현에는 Host/Node용 `common_hosts`만 반영되고 Pod용 `coredns_records` 자동화가 빠져 있었다.

따라서 문제의 핵심은 새로운 DNS 구조가 필요했던 것이 아니라 **설계에 있던 Pod용 프로젝트 도메인 등록 자동화가 구현에서 누락된 것**이었다.
...
| 후보 | 장점 | 단점 | 판정 |
|---|---|---|---|
| **CoreDNS `hosts` + Ansible `coredns_records`** | 기존 05·06 설계와 일치, 추가 서버 없음, Pod 전체 공통 적용, Ansible 재현·멱등성 검증 가능 | CoreDNS 전체 Pod에 영향을 줄 수 있어 변경 후 DNS 회귀검증 필요 | **채택** |
...
Pod별 `hostAliases`나 BuildKit에만 별도 hosts 설정을 넣는 방식은 특정 Pod만 우회하고 같은 문제가 다른 Pod에서 반복될 수 있어 공식 해결안에서 제외했다.

## 조치

### 1. 프로젝트 도메인 정보를 한 곳에서 관리

공용 `project_endpoints` 정의를 만들고 Host와 Pod가 같은 FQDN/IP 값을 사용하도록 정리했다.

현재 CoreDNS에 등록하는 도메인:
...
`k8s-api`, Jenkins, Argo CD 외부 주소처럼 당시 아직 SAN·Route·MVP 필요성이 확정되지 않은 항목은 이름이 존재한다는 이유만으로 미리 등록하지 않았다.

### 2. Host와 Pod의 이름해석 설정을 분리
...
이 구조로 동일 FQDN/IP를 `/etc/hosts`와 CoreDNS에 따로 적어 두다가 값이 달라지는 문제를 피했다.

### 3. CoreDNS 변경 범위를 제한

CoreDNS 전체 Corefile을 새 Template로 덮어쓰지 않고, 프로젝트가 관리하는 `hosts` Block만 제한적으로 수정하도록 구현했다.

안전장치로 다음을 포함했다.

- 현재 Corefile과 관리 구간 확인
- 변경 전 Backup
- 적용 전에 생성될 Corefile 내용 검증
- 프로젝트가 관리하는 Block만 수정
- 실제 변경이 있을 때만 CoreDNS Rollout
- Kubernetes Service DNS 재검증
- 프로젝트 도메인 DNS 검증
- 외부 DNS 재검증
- 실패 시 진단 정보 수집 및 이전 Corefile 복구

## 구현 과정에서 발생한 별도 장애

최초 적용 과정에서 CoreDNS 관리 Block의 줄바꿈 처리 오류로 생성된 Corefile 구조가 깨져 신규 CoreDNS Pod가 `CrashLoopBackOff`가 되는 별도 문제가 발생했다.
...
Host `/etc/hosts`의 프로젝트 도메인 배포도 관리 대상 Host에서 재실행 `changed=0 / failed=0 / unreachable=0`을 확인했다.
...
`seokpan-gitops#21`에서 기존 Jenkins Job을 다시 실행했을 때 기존 DNS `SERVFAIL`은 더 이상 재현되지 않았고, 다음 단계에서 `x509: certificate signed by unknown authority`가 새로 나타났다. 따라서 DNS 문제는 실제 BuildKit Agent에서도 해소됐음을 확인했다.

Harbor 내부 CA 신뢰와 이후 Robot Account 인증 문제는 DNS와 원인이 다르므로 별도 사례로 관리한다.

## Before → Change → After

```text
Before
Host/Node는 /etc/hosts로 프로젝트 도메인 해석 가능
→ Pod는 CoreDNS 사용
→ 상위 DNS에 프로젝트 Record 없음
→ BuildKit Pod에서 Harbor FQDN SERVFAIL

Change
프로젝트 FQDN/IP 값을 project_endpoints에서 공통 관리
→ common_hosts와 coredns_records가 같은 값을 사용
→ CoreDNS의 hosts Block만 Ansible로 제한 수정
→ 변경 전 Backup·적용 전 검증·실패 시 복구 절차 추가

After
Pod에서 프로젝트 도메인 DNS 정상
→ Kubernetes Service DNS와 External DNS 이상 없음
→ Ansible 2회차 changed=0
→ BuildKit 재실행에서 기존 SERVFAIL 미재발
→ 다음 단계인 Harbor TLS 인증서 검증으로 진행
```
