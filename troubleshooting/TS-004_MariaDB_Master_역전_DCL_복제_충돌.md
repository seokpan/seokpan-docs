...
# TS-004 — MariaDB Backup 검증 중 실제 Master 역전과 DCL 복제 충돌로 SQL Thread 정지
...
| **영향 역할** | 정태훈(애플리케이션의 DB 사용 경로), 이유빈(Inventory), DB를 사용하는 전체 구성요소 |
| **핵심 범주** | MariaDB / Replication / Failover / DCL / Recovery |
...
1. 문서상/초기 Inventory에서 `mariadb-01`을 Primary라고 고정해 생각하고 있었으나, 실제 Runtime은 MaxScale Auto Failover 때문에 `mariadb-02`가 Master로 승격돼 있었다.
...
**DB에 쓰기 작업을 하기 전에 문서나 호스트 이름으로 Master를 추정하지 않고 `maxctrl list servers`로 실제 Runtime Master를 확인한다.**
