...
| **시점** | 2026-08-31 |
| **상태** | **예방형 · 해결** |
| **주 담당** | **김상희 — Storage / 정태훈 — Runtime·Review Validation** |
| **영향 역할** | Kubernetes Cluster 권한 경계 전체 |
| **핵심 범주** | Kubernetes RBAC / Least Privilege / Review Correction |
| **Evidence** | Review 예방 + 실제 Storage Runtime 후속 검증 |
...
## PR Review에서 판단이 뒤집힌 이유

실제 GitOps PR #13을 Review하면서 현재 사용하는 NFS Subdir External Provisioner v4.0.2의 동작과 필요한 권한을 다시 대조했다. 그 결과 `nodes get/list/watch`를 프로젝트의 상시 ClusterRole에 넣어야 한다는 근거가 충분하지 않다고 판단했다.

Cluster-scoped 권한은 Argo CD Desired State에 들어가면 지속적으로 유지되므로, “Upstream에 있었던 것으로 보인다”는 이유만으로 권한을 추가하는 것은 최소권한 원칙에 맞지 않았다.
...
- `archiveOnDelete`는 Runtime 데이터 보존 관점으로 별도 확정

## 후속 검증

Review 수정 후 Storage Runtime 단계에서 다음 경로를 확인했다.
...
이 사례는 단순 RBAC 수정이 아니라 **초기 기술 판단도 Evidence가 부족하면 Review에서 뒤집어야 한다는 사례**다.
...
- 후속 Storage Evidence: https://github.com/seokpan/seokpan-infra/issues/44
